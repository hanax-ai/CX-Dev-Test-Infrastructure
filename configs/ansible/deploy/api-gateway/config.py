...
class GatewayConfig:
    ...
    """
    def __init__(self, config_path="config.yaml", raise_on_error=True):
        self.model_map = {}
        self.backend_hosts = []
    ...
    def reload_config(self, config_path="config.yaml", raise_on_error=None):
        ...
        # Temporarily set the raise_on_error behavior for this reload
        original_raise_on_error = self.raise_on_error
        self.raise_on_error = raise_on_error
        try:
            self._load_config(config_path)
        finally:
